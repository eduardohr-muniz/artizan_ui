...
The benefits of using `DSForm` over managing form fields individually are:
...
You can set the initial form value by passing a `Map<String, dynamic>` to the `initialValue` property of the `DSForm` widget.
...
You can get the current form value by accessing the `value` property of `DSFormState` using a `GlobalKey`.
...
You can set or update the value of specific form fields using the `setFieldValue` method of `DSFormState`.
...
You can set or update the entire form value using the `setValue` method of `DSFormState`.
...
If your `DSForm` has an initial value like this one `{'date': '2024-02-01'}` and you need to convert the string value to a `DateTime` object for a `DSDatePickerFormField`:
```dart {3}
...
By default, `DSForm` supports dot notation in field IDs to automatically create nested map structures. This makes it easier to work with complex, hierarchical form data.
...
See the following links for more examples on how to use the `DSForm` component with other components:
